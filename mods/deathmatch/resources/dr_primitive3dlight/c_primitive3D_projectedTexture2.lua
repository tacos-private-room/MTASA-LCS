--  
-- file: c_primitive3D_projectedTexture2.lua
-- version: v1.6
-- author: Ren712
--
-- include: fx/primitive3D_projectedTexture2.fx, c_common.lua

local scx,scy = guiGetScreenSize ()

CPrmTextureProj2 = { }
CPrmTextureProj2.__index = CPrmTextureProj2

-- texture, worldPosition Vector3(), elementRotation Vector3(), texSize Vector2(), color Vector4(0-255,0-255,0-255,0-255)
function CPrmTextureProj2: create(tex, pos, atten, rot, texSiz, col )
	local scX, scY = guiGetScreenSize()
	
	local cShader = {
		shader = DxShader( "fx/primitive3D_projectedTexture2.fx" ),
		texture = tex,
		textureSize = texSiz,
		lightNearClip = 0,
...
function CPrmTextureProj2: setTesselationByDistance()
...
function CPrmTextureProj2: setDirection( dir )
	if self.shader then
...
function CPrmTextureProj2: setRotation( rot )
	if self.shader then
...
function CPrmTextureProj2: setTexture( tex )
	if self.shader then
		self.texture = tex
...
function CPrmTextureProj2: setTextureSize( texSize )
	if self.shader then
...
function CPrmTextureProj2: setLightNearClip( lNearClip )
	if self.shader then
...
function CPrmTextureProj2: setDistFade( distFade )
	if self.shader then
...
function CPrmTextureProj2: setPosition( pos )
	if self.shader then
...
function CPrmTextureProj2: setAttenuation( atten )
	if self.shader then
...
function CPrmTextureProj2: setAttenuationPower( attenPow )
	if self.shader then
...
function CPrmTextureProj2: setColor( col )
	if self.shader then
...
function CPrmTextureProj2: getObjectToCameraAngle()
...
function CPrmTextureProj2: getDistanceFromViewAngle( inAngle )
	if self.shader then
...
function CPrmTextureProj2: draw()
	if self.shader then
...
function CPrmTextureProj2: destroy()
	if self.shader then
		self.shader:destroy()
		self.shader = nil
	end
