	...
	for v,_ in ipairs(GUISP.zRemoveList[w]) do if GUISP.zRemoveList[w]=={} then return end
		for i,_ in ipairs(GUISP.optionList[w]) do  
			if guiComboBoxGetItemText(GUISP.combo[w], i)==GUISP.zRemoveList[w][v] then 
				guiComboBoxRemoveItem (GUISP.combo[w], i ) backHeight=backHeight-14 
				end 
			end
		end
...
function enablePanelFx()	
    ...
    if 	isDepthBufferAccessible()==false then removeDepthBufferOptions() end
	
	if Settings.var.PaletteON~=0 then palette = dxCreateTexture ( "images/enbpalette"..Settings.var.PaletteON..".bmp" ) end

	carGrunLiRefS = dxCreateShader ( "shaders/car_refgrunL.fx",1,Settings.var.renderDistance,true)
	carGrunStRefS = dxCreateShader ( "shaders/car_refgrun.fx",1,Settings.var.renderDistance,false)
    carGeneShader = dxCreateShader ( "shaders/car_refgene.fx",1,Settings.var.renderDistance,true)
    carShatShader = dxCreateShader ( "shaders/car_refgene.fx",1,Settings.var.renderDistance,true)
    carPaintShader = dxCreateShader ( "shaders/car_paint.fx" )
    waterClaShader = dxCreateShader ( "shaders/water.fx" )
	waterRefShader = dxCreateShader ( "shaders/water_ref.fx" )
...
function setEffectType1()
    ...
    v.BloomIntensity=0.24
    v.BloomSaturation=0.66
    v.BaseIntensity=0.24
    v.BaseSaturation=-0.18
	...
	v.BloomIntensityBL=0.5 
	v.BaseIntensityBL=0.5
...
-----------------------------------------------------------------------------------
	...
	function carpaint_staRef()
			...
			if Settings.var.CarPaintVers==3 then carGrunShader=carGrunLiRefS end
			if Settings.var.CarPaintVers==2 then carGrunShader=carGrunStRefS end
	...
	if Settings.var.WaterVers == 2 then waterShader=waterRefShader 
	...
	if Settings.var.WaterVers == 1 then waterShader=waterClaShader end
...
addEventHandler( "onClientHUDRender", root,
    function()
		...
		current1 = applyPalette(current1,v.PaletteR,v.PaletteG,v.PaletteB,v.PaletteA ) 
		end
		...
		local current_temp=current2
		if v.ContrastON==1 then current2 = applyDownsampleSteps( current2, v.DownSampleSteps ) 
		end
		if v.BloomON==1 then
		 current2 = applyDownsample( current_temp )
		 current2 = applyDownsample( current_temp )
		end
		...
		if v.ContrastON==1 and v.BloomON~=1 then 
		  ...
		  if (v.PaletteON)>0 then 
		   current2 = applyPalette(current2,v.PaletteR,v.PaletteG,v.PaletteB,v.Palettea ) 
	      end
		end
...
function applyPalette( Src,PaletteR,PaletteG,PaletteB,PaletteA )
	if not Src then return nil end
	local mx,my = dxGetMaterialSize( Src )
	local newRT = RTPool.GetUnused(mx,my)
	if not newRT then return nil end
    ...
    dxSetShaderValue( paletteShader, "gCol", PaletteR,PaletteG,PaletteB,PaletteA )
