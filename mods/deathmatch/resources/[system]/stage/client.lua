...
function ShowHUD()
setPlayerHudComponentVisible ( "ammo", showHUD )
setPlayerHudComponentVisible ( "area_name", showHUD )
setPlayerHudComponentVisible ( "armour", showHUD )
setPlayerHudComponentVisible ( "breath", showHUD )
setPlayerHudComponentVisible ( "clock", showHUD )
setPlayerHudComponentVisible ( "health", showHUD )
setPlayerHudComponentVisible ( "money", showHUD )
setPlayerHudComponentVisible ( "radar", showHUD )
setPlayerHudComponentVisible ( "vehicle_name", showHUD )
setPlayerHudComponentVisible ( "weapon", showHUD )
showChat(showHUD)
...
function SaveFirePts()	
	if ((getPedControlState("aim_weapon")==false) and (getPedControlState("fire")==false)) then
...
a = getPedControlState("aim_weapon")
b = getPedControlState("fire")
-- tmpFirePts
-- fpcnt
if (a or b) then
...
--abz1 = abz
	if (isPedInVehicle (locPlr)==false) then-- on foot
		...
		if (occ==locPlr) then
			...
			tmpKeyFrame[kfcnt][6],tmpKeyFrame[kfcnt][7],tmpKeyFrame[kfcnt][8] = getElementAngularVelocity (veh)
		...
		if (nt>par["kfrm"][Pkfcnt]["d"]) then
			...
			--	abz1 = getPedRotation(locPlr)
				...
				--	else
					...
					setElementAngularVelocity (veh, tvx,tvy,tvz)
	...
	if (isPedInVehicle (locPlr)==false) then-- on foot
			...
			if (veh~=false) then
				...
				if (occ==locPlr) then
					...
					AdCurr[6],AdCurr[7],AdCurr[8] = getElementAngularVelocity (veh)
