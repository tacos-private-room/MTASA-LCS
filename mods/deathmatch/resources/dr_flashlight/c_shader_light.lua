...
addEventHandler("onClientPreRender", root, function()
	for index,this in ipairs(getElementsByType("player")) do
		if flashLiTable.shLight[this] then
			...
			exports.dr_lightmanager:setLightDirection(flashLiTable.shLight[this],lx1-x1,ly1-y1,lz1-z1)
			exports.dr_lightmanager:setLightPosition(flashLiTable.shLight[this],x1,y1,z1)	
		end
	end
end
, true, "high+10")
...
function createWorldLight(thisPed)
	...
	if (thisPed~=localPlayer) then
		flashLiTable.shLight[thisPed] = exports.dr_lightmanager:createSpotLight(0,0,3,gLightColor[1] * 255,gLightColor[2] * 255,gLightColor[3] * 255,gLightColor[4] * 255,0,0,0,gLightFalloff,gLightTheta,gLightPhi,gAttenuation)
	else
		flashLiTable.shLight[thisPed] = exports.dr_lightmanager:createSpotLight(0,0,3,gLightColor[1] * 255,gLightColor[2] * 255,gLightColor[3] * 255,gLightColor[4] * 255,0,0,0,gLocLightFalloff,gLocLightTheta,gLocLightPhi,gLocAttenuation)
	end
end

function destroyWorldLight(thisPed)
	if flashLiTable.shLight[thisPed] then
		flashLiTable.shLight[thisPed] = not exports.dr_lightmanager:destroyLight(flashLiTable.shLight[thisPed])
	end
end
