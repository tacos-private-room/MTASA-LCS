...
addEventHandler("requestServerData", resourceRoot,
	function()
		triggerClientEvent(client, "receiveServerData", resourceRoot, hangingWeightLimit, showChatMessages, chatColour)
	end
)
...
function setPlayerGrabState(player, state, vehicle, side, linePercent)
	...
	if player and isElement(player) then
		if state == true or state == "true" then
			...
			if
				vehicle and isElement(vehicle) and getElementType(vehicle) == "vehicle" and getVehicleType(vehicle) == "Helicopter"
			 then
				side = side or 1
...
function getPlayersGrabbingHeli(vehicle, side, linePercent)
	if vehicle then
		...
		for _, player in ipairs(getElementsByType("player")) do
			local hangVehicle = getElementData(player, "heligrab.vehicle")
			if hangVehicle then
				if (vehicle == hangVehicle and ((not side) or (side == getElementData(player, "heligrab.side"))) and
						((not linePercent) or (linePercent == getElementData(player, "heligrab.linePercent")))) then
...
addEventHandler("onPlayerVehicleExit", root,
	function(vehicle, seat, jacker, forcedByScript)
		...
		if getVehicleType(vehicle) == "Helicopter" and seat == 0 then
			...
			local nearbyPlayer = getElementsWithinRange(x, y, z, 30, "player")
			if nearbyPlayer and table.empty(nearbyPlayer) == false then
				for i, p in ipairs(nearbyPlayer) do
					local hangVehicle = getElementData(p, "heligrab.vehicle")
	...
	function()
		for _, player in ipairs(getElementsByType("player")) do
			...
			if hangVehicle then
				...
				setElementData(player, "heligrab.interior", nil)
				setElementData(player, "heligrab.dimension", nil)
			end
		end
	end
)
