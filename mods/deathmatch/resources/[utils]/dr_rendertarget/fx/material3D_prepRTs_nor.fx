// 
// file: material3D_prepRTs_nor.fx
// version: v1.6
// author: Ren712
//
...
float3 GetSmoothedNormals(float2 texcoord, float3 ScreenSpaceNormals, float3 ScreenSpacePosition, float4 uvToView)
{
	...
	for(float x = -3; x <= 3; x++)
	{
		...
		for(float y = -3; y <= 3; y++)
		{
			float2 offsetcoord 	= texcoord.xy + float2(x,y) * depthFac * sPixelSize * 3.4999f;
...
Pixel PixelShaderFunction(PSInput PS)
{
    ...
    if ((depthVal > (depth + 0.0001))) outColor = 1;
    if (((depthVal > (depth + 0.0001))) || (length(normalIn) > 1.01)) 
    {
    ...
    float distFromCam = Linearize(FetchDepthBufferValue(TexProj.xy));
    ...
    outColor.rgb = outColor.rgb * 0.9 + 0.1;
...
// Techniques
//--------------------------------------------------------------------------------------
technique material3D_prepRTs_nor
{
  pass P0
  {
    ZEnable = false;
...
technique material3D_prepRTs_nor_noDB
{
  pass P0
  {
    ZEnable = false;
